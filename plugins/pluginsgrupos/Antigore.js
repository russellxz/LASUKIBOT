// plugins/pluginsgrupos/Antigore.js — activa el filtro de sangre / gore
import fs from 'fs';
...
const handler = async (msg, { conn }) => {
  ...
  if (!["on", "off"].includes(estado)) {
    const activo = parseInt(getConfig(chatId, "antigore")) === 1;
...
infalible.`
  ...
  if (estado === "on") {
    setConfig(chatId, "antigore", 1);
  } else {
    deleteConfig(chatId, "antigore");
  }
