...
const handler = async (msg, { conn, args, text, wa }) => {
  ...
  if (!textoCrudo && !quotedText && !quotedImage) {
    return conn.sendMessage(
      chatId,
      { text: "✏️ Usa: *setlotes <texto>* (multilínea) o responde a una *imagen* con: *setlotes <texto>*" },
      { quoted: msg }
    );
  }
  ...
  if (quotedImage) {
    try {
      ...
      console.error("[setlotes] imagen:", e);
  ...
  db[chatId]["setlotes"] = { texto: textoFinal, imagen: imagenBase64 };
  saveJsonAtomic(db);

  await conn.sendMessage(chatId, { text: "✅ *LOTES actualizado con éxito.*" }, { quoted: msg });
};
