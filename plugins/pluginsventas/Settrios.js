...
const handler = async (msg, { conn, args, text, wa }) => {
  ...
  if (!textoCrudo && !quotedText && !quotedImage) {
    return conn.sendMessage(
      chatId,
      { text: "✏️ Usa: *settrios <texto>* (multilínea) o responde a una *imagen* con: *settrios <texto>*" },
      { quoted: msg }
    );
  }
  ...
  if (quotedImage) {
    try {
      ...
      console.error("[settrios] imagen:", e);
  ...
  db[chatId]["settrios"] = { texto: textoFinal, imagen: imagenBase64 };
  saveJsonAtomic(db);

  await conn.sendMessage(chatId, { text: "✅ *TRIOS actualizado con éxito.*" }, { quoted: msg });
};

handler.command = ["settrios"];
