...
// helpers comunes (mismos que arriba)
...
const handler = async (msg, { conn, args, text, wa }) => {
  ...
  if (!textoCrudo && !quotedText && !quotedImage) {
    return conn.sendMessage(
      chatId,
      { text: "✏️ Usa: *setstock2 <texto>* (multilínea) o responde a una *imagen* con: *setstock2 <texto>*" },
      { quoted: msg }
    );
  }
  ...
  db[chatId]["setstock2"] = { texto: (textoCrudo || quotedText || ""), imagen: imagenBase64 };
  saveJsonAtomic(db);

  await conn.sendMessage(chatId, { text: "✅ *STOCK2 actualizado con éxito.*" }, { quoted: msg });
};
