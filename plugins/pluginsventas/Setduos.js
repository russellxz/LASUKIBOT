...
const handler = async (msg, { conn, args, text, wa }) => {
  ...
  if (!textoCrudo && !quotedText && !quotedImage) {
    return conn.sendMessage(
      chatId,
      { text: "✏️ Usa: *setduos <texto>* (puede ser multilínea) o responde a una *imagen* con: *setduos <texto>*" },
      { quoted: msg }
    );
  }
  ...
  if (quotedImage) {
    try {
      ...
      console.error("[setduos] imagen:", e);
  ...
  db[chatId]["setduos"] = { texto: textoFinal, imagen: imagenBase64 };
  saveJsonAtomic(db);

  await conn.sendMessage(chatId, { text: "✅ *DUOS actualizado con éxito.*" }, { quoted: msg });
};

handler.command = ["setduos"];
