// subplugins/grupos/welcome.js — Activa/desactiva las bienvenidas del SUBBOT
...
const handler = async (msg, { conn, args }) => {
  ...
  if (!["on", "off"].includes(estado)) {
    return conn.sendMessage(chatId, { text: `✳️ Usa correctamente:\n\n${p}welcome on / off` }, { quoted: msg });
  }

  const data = conn.readSubData("welcome.json", {});
  if (!data[chatId]) data[chatId] = {};
  if (estado === "on") data[chatId].welcome = 1;
  else delete data[chatId].welcome;
  conn.writeSubData("welcome.json", data);

  await conn.sendMessage(chatId, {
    text: `🎉 Bienvenidas del subbot *${estado === "on" ? "activadas" : "desactivadas"}* correctamente.`
  }, { quoted: msg });
